## Publications

*Equal Contribution*

### Conference

1. [Reasoning Models Better Express Their Confidence](https://arxiv.org/abs/2505.14489)  
   **[Dongkeun Yoon]**, Seungone Kim, Sohee Yang, Sunkyoung Kim, Soyeon Kim, Yongil Kim, Eunbi Choi, Yireun Kim, Minjoon Seo  
   *NeurIPS 2025*

2. [M-Prometheus: A Suite of Open Multilingual LLM Judges](https://arxiv.org/abs/2504.04953)  
   José Pombal, **[Dongkeun Yoon]**, Patrick Fernandes, Ian Wu, Seungone Kim, Ricardo Rei, Graham Neubig, André FT Martins  
   *COLM 2025*

3. [The BiGGen Bench: A Principled Benchmark for Fine-grained Evaluation of Language Models with Language Models](https://arxiv.org/abs/2406.05761)  
   Seungone Kim, Juyoung Suk, Ji Yong Cho, Shayne Longpre, Chaeeun Kim, **[Dongkeun Yoon]**, …  
   *NAACL 2025 (🎉 Best Paper)*

4. [LangBridge: Multilingual Reasoning Without Multilingual Supervision](https://arxiv.org/abs/2401.10695)  
   **[Dongkeun Yoon]**, Joel Jang, Sungdong Kim, Seungone Kim, Sheikh Shafayat, Minjoon Seo  
   *ACL 2024*

5. [Gradient Ascent Post-training Enhances Language Model Generalization](https://arxiv.org/abs/2306.07052)  
   **[Dongkeun Yoon*]**, Joel Jang*, Sungdong Kim, Minjoon Seo  
   *ACL 2023*

6. [Knowledge Unlearning for Mitigating Privacy Risks in Language Models](https://arxiv.org/abs/2210.01504)  
   Joel Jang, **[Dongkeun Yoon]**, Sohee Yang, Sungmin Cha, Moontae Lee, Lajanugen Logeswaran, Minjoon Seo  
   *ACL 2023*

7. [Improving Graph-based Document-Level Relation Extraction Model with Novel Graph Structure](https://dl.acm.org/doi/abs/10.1145/3511808.3557615)  
   Seongsik Park, **[Dongkeun Yoon]**, Harksoo Kim  
   *CIKM 2022*

### Technical Report

1. [K-EXAONE Technical Report](https://arxiv.org/abs/2601.01739v1)  
   LG AI Research  
   *Jan 2026*

### Preprint

1. [A 2-step Framework for Automated Literary Translation Evaluation: Its Promises and Pitfalls](https://arxiv.org/abs/2412.01340)  
   Sheikh Shafayat, **[Dongkeun Yoon]**, Woori Jang, Jiwoo Choi, Alice Oh, Seohyon Jung  
   *arXiv preprint, Dec 2024*

2. [MM-Eval: A Multilingual Meta-Evaluation Benchmark for LLM-as-a-Judge and Reward Models](https://arxiv.org/abs/2410.17578)  
   Guijin Son*, **[Dongkeun Yoon*]**, Juyoung Suk, Javier Aula-Blasco, Mano Aslan, Vu Trong Kim, …  
   *arXiv preprint, Oct 2024*
